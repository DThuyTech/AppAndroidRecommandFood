...
//            Log.d("test", tr.logicfunc(mListUser,saveAnswer));
            ...
            List<Food> foodsend = new ArrayList<Food>();

            foods = AppDatabase.getInstance(this).foodDao().getAllFood();
            Train tr = new Train();
            foods = tr.logicfunc(mListUser,saveAnswer,foods);

            Intent myIntent = new Intent(this, ShowFood.class);
            Bundle bundle = new Bundle();
            bundle.putInt("Mon1",foods.get(0).getId() );
            bundle.putInt("Mon2",foods.get(1).getId());
            Log.d("test", ""+foods.get(0).getId()+" "+foods.get(1).getId());
